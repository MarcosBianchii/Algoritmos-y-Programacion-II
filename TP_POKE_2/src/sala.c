...
*/ 
{
	...
	lista_insertar(sala->interacciones, inter);
	return EXITO;
}
...
//	nombres de los objetos, solo sirve para los abbs.
*/
{
...
*/
{
	...
	||   abb_buscar(sala->conocidos, inter->objeto))
...
{
	if (abb_buscar(sala->poseidos, inter->objeto)){
		abb_quitar(sala->poseidos, inter->accion.objeto);
		free(hash_quitar(sala->objetos, inter->accion.objeto));
...
static int ejecutar_comandos_objetos(sala_t *sala, const char *verbo, const char *objeto1,
				     void (*mostrar_mensaje)(const char *mensaje,
			   				     enum tipo_accion accion,
							     void *aux),
				     void *aux)
			      ...
			      void *aux)
{
	...
	if (ejecutar_comandos_objetos(sala, verbo, objeto1, mostrar_mensaje, aux) == 1)
		return 1;
